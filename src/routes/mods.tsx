...
import { PLAYER_MODS, LIBRARY_MODS, CATEGORIES, CATEGORY_ORDER } from '../data/mods'
import type { Mod, Category } from '../data/mods'
...
function ModsPage() {
  ...
  const [activeCat, setActiveCat] = useState<Category | 'all'>(initCat ?? 'all')
  const [selectedMod, setSelectedMod] = useState<Mod | null>(null)
  const [showLibs, setShowLibs] = useState(false)

  const filtered = useMemo(() => {
    let pool = PLAYER_MODS
    if (activeCat !== 'all') pool = pool.filter((m) => m.category === activeCat)
  ...
  return (
    <main className="page-wrap" style={{ paddingTop: '3rem', paddingBottom: '5rem' }}>

      {/* Page title */}
      <div style={{ marginBottom: '2rem' }}>
        <p className="label" style={{ marginBottom: '0.4rem' }}>TSW Fabric</p>
        <h1 className="display" style={{ fontSize: '2rem', marginBottom: '0.4rem' }}>
          Mod Browser
        </h1>
        ...
        <button
          ...
          style={{
            ...
            ...(activeCat === 'all'
              ...
              : { background: 'transparent', borderColor: 'var(--border)', color: 'var(--text-soft)' }),
          }}
        >
      ...
      {filtered.length === 0 ? (
        ...
        >
          ...
          <p style={{ margin: 0, fontWeight: 600, color: 'var(--text)' }}>No mods found</p>
        ...
        >
          <div>
            ...
            <p style={{ margin: '0.2rem 0 0', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
              Behind-the-scenes libraries — you don't interact with these directly
            </p>
          </div>
          <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{showLibs ? '▲' : '▼'}</span>
        </button>
        ...
        {showLibs && (
          ...
          >
            {LIBRARY_MODS.map((mod) => (
              ...
              >
                <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{mod.name}</span>
                {mod.modrinth && (
                  <a
