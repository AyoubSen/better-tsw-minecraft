...
export const Route = createFileRoute("/directed-guides")({
	validateSearch: (s: Record<string, unknown>): Search => ({
		category:
			typeof s.category === "string" && isGuideCategory(s.category)
				? s.category
				: undefined,
...
const CATEGORIES = [
	"All",
	...Array.from(new Set(MOD_GUIDES.map((g) => g.category))),
];
...
function haystack(g: ModGuide) {
	return normalize(
		[g.title, g.category, g.tagline, ...g.mods, ...g.searchTerms].join(" "),
	);
}
...
function DirectedGuidesPage() {
	...
	const filtered = useMemo(
		() =>
			MOD_GUIDES.filter((g) => {
				...
				const matchesQ =
					!normalizedQuery || haystack(g).includes(normalizedQuery);
	...
	return (
		<main
			className="page-wrap"
			style={{ paddingTop: "3rem", paddingBottom: "5rem", maxWidth: 1100 }}
		>
			{/* Header */}
			<section
				className="fade-up"
			...
			>
				...
				>
					Search by mod name or problem. Essential guides first — start there if
					you're unsure.
				</p>
			</section>
				...
				{CATEGORIES.map((cat) => (
					<button
						...
						style={{
							...
							color:
								activeCategory === cat ? "var(--accent)" : "var(--text-soft)",
							fontSize: "0.74rem",
							fontWeight: 700,
						}}
					>
